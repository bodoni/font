...
tests-draw-selected:
	cargo run --bin font-draw --features bin --quiet -- \
...
tests-name-selected:
	cargo run --bin font-name --features bin --quiet -- \
		--font tests/fixtures/web-platform-tests/fonts/CSSTest/csstest-ascii.ttf

tests-sign-selected:
	RUST_BACKTRACE=full cargo run --bin font-sign --features bin -- \
...
tests-scan:
	...
	RUST_BACKTRACE=full cargo run --bin font-scan --features bin -- \
		--path tests/fixtures \
